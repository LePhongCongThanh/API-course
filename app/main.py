...
# get method
# Path operation
@app.get("/") # giống flask, khai báo phương thức get và url o day la root
...
@app.get("/posts") # cũng là get nhưng URLs khác
...
@app.post("/posts", status_code=status.HTTP_201_CREATED) # thay doi status code
...
@app.get("/posts/{ID}") #1 The "{ID}" part is a path parameter that allows you to specify the ID of the post in the URL.
...
@app.get("/db/posts/{ID}") #1 The "{ID}" part is a path parameter that allows you to specify the ID of the post in the URL.
def get_posts(ID: int): #2
...
@app.delete("/posts/{ID}", status_code=status.HTTP_204_NO_CONTENT) # path parameter
...
# Post method
@app.post("/createpost") # phuong thức post user send data to API server
